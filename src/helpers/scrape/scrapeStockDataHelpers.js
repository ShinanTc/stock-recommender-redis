...
export async function collectStockInformation(page) {
  ...
  try {
    ...
    let noRecordsElt = await page.waitForXPath(
      "/html/body/app-root/div/app-stock-view-details/div/div/div[1]/div/div/div[3]/div/p"
    ); // checking if there is a No record found message

    if (noRecordsElt) return { stocks, numberOfPagesScraped };
  } catch (error) {
    ...
    while (true) {
      ...
      if (i === 1) numberOfStocksInCurrentPage = 1;
      ...
      if (i <= 21) element = await waitForXPathAndReturnElements(page, xPaths);
      else element = "TimeoutError";
      ...
      if (stockDetails !== undefined) stocks.push(stockDetails);
...
export async function validateScrape(page, stockDetails) {
  ...
  for (var i = 0; i < 3; i++) {
    if (stockDetails.scrapeCount < 5) {
      let { stocks, numberOfPagesScraped } = await collectStockInformation(
        page
      );
